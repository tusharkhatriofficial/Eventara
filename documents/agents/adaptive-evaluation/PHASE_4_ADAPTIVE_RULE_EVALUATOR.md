...
**Goal**: Implement the core service that manages dynamic ticking and optimized grouped evaluation.  
**Duration**: ~2 hours  
**Difficulty**: **Hard** (Core Logic)

---

## Prerequisites

- [x] Phase 1 completed (`AdaptiveEvaluationProperties`)
- [x] Phase 2 completed (`EventRateMonitor`)
- [x] Phase 3 completed (`EvaluationKey`)
- [x] Project compiles successfully

---

## Step 1: Create AdaptiveRuleEvaluator.java

**File Path:** `src/main/java/com/eventara/rule/evaluation/AdaptiveRuleEvaluator.java`

Create this file and paste the following code. Use the copy button to ensure you get the full file.
...
public class AdaptiveRuleEvaluator {
    ...
    // Virtual metrics that shouldn't be evaluated here (handled elsewhere or complex)
    ...
    private void evaluateRuleAgainstBucket(AlertRule rule, MetricsBucket bucket, int windowMinutes) {
        ...
        if (configMap.containsKey("conditions")) return;
        ...
        if (metricType == null || isVirtualMetric(metricType)) return;
        ...
        if (condition == null || thresholdObj == null) return;
    ...
    public synchronized void refreshRuleCache() {
        try {
            ...
            if (cachedRules == null) cachedRules = List.of();
        }
    }
...
```
   ...
   └─────────┬────────────┘
             ...
             ▼
   ┌──────────────────────┐      ┌─────────────────────────────┐
   │ Evaluate Rules       │ ◄─── │ Group Rules (EvaluationKey) │
   │ 1. Group Rules       │      └─────────────────────────────┘
   │ 2. Fetch Metrics     │             │
   │ 3. Check Thresholds  │             ▼
   │ 4. Fire Alerts       │      ┌─────────────────────────────┐
   └─────────┬────────────┘      │ Fetch Metric Bucket (Redis) │
             │                   └─────────────────────────────┘
             ▼
   ┌──────────────────────┐
   │ Adjust Interval      │ ◄─── (From RateMonitor)
   │ - High EPS → Fast    │
   │ - Low EPS  → Slow    │
   └──────────────────────┘
```
...
- [x] Created `AdaptiveRuleEvaluator.java`
- [x] Implemented the core dynamic ticking logic
- [x] Implemented the optimized grouped evaluation

---

## Next Step

Proceed to **Phase 5: Integration with EventConsumer** to actually wire this into the event stream!

---

*Phase 4 Complete | Total Est. Time: 2 hours*
